...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        next_task.have_start = true;
        next_task.task_start_time = get_time_us();
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            if inner.tasks[next].have_start==false{
                inner.tasks[next].task_start_time=get_time_us();
                inner.tasks[next].have_start=true;
            }
    ...
    fn get_current(&self) -> TaskControlBlock
    {
        let inner = self.inner.exclusive_access();
        let curr_id=inner.current_task;
        let tskctrblock=TaskControlBlock { 
